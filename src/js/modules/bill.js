...
var bill = {
	...
	onTimeQuantityChange: (
		...
		$changedSelectric
			.find(".selectric-scroll li[data-index=0]")
			.text(quantityToTime[0]);
		$changedSelectric
	...
	onBackspaceKeydown: (evt, currentElem) => {
		if (
			evt.keyCode === bill.BACKSPACE_KEYCODE &&
			currentElem.value.length === 1
		) {
			bill.prevValue = "";
	...
	onBillFormSubmit: () => {
		messageSend.onFormSubmit(true);
		messageSend.setSendTextInPreItem("Выставили счёт");
	...
	init: () => {
		...
		bill.$quantitySelect.on("change", function () {
			bill.onTimeQuantityChange(
				bill.$quantitySelect,
				bill.$hoursDaysSelect,
				bill.$hoursDaySelectric
			);
		});
