...
var setDialogContent = {
	...
	getMessageDateTime: (dateTime) => {
		...
		const messageMinutes =
			messageDateTime.getMinutes() < 10
				? `0${messageDateTime.getMinutes()}`
	...
	makeAttachedFiles: (filesDataArr, $messageElem) => {
		filesDataArr.forEach((current) => {
			if (current["type"] !== "video") {
				...
				$attachedFile.attr(
					"href",
					`${setDialogContent.FILE_SRC}${current["name"]}`
				);
	...
	makeVideo: (filesDataArr, $messageElem) => {
		...
		filesDataArr.forEach((current) => {
			if (current["type"] === "video") {
				const $item = $(
					$(setDialogContent.init.$videoItemTemplate)[0].cloneNode(true)
				);
	...
	makeMessage: (
		...
		setDialogContent.setAuthorName(
			authorName,
		...
		if (messageData["files"].length > 0) {
			setDialogContent.makeAttachedFiles(
				messageData["files"],
				$messageTextWrap
			);
		...
		if (messageData["bill"]) {
			if (messageData["messageAuthor"] === "me") {
				$messageText.append(
					setDialogContent.makeBill(
						messageData["bill"],
						setDialogContent.init.$myBillTemplate
					)
				);
			} else {
				$messageText.append(
					setDialogContent.makeBill(
						messageData["bill"],
						setDialogContent.init.$userBillTemplate
					)
				);
			}
		}
	...
	makeMessages: (authorName, avatarData, messagesDataArr, dialogList) => {
		...
		messagesDataArr.forEach((current) => {
			if (current["messageAuthor"] === "customer") {
				dialogList.append(
					setDialogContent.makeMessage(
						authorName,
						avatarData,
						current,
						setDialogContent.init.$userMessageTemplate
					)
				);
			} else {
				dialogList.append(
	...
	init: () => {
		...
		setDialogContent.init.$userMessageTemplate = $("#user-message")
			.contents()
			.find(".dialog__message");
		setDialogContent.init.$myMessageTemplate = $("#my-message")
			.contents()
			.find(".dialog__message");
		setDialogContent.init.$attachedFileTemplate = $(
			"#attached-file"
		).contents();
		setDialogContent.init.$videoContainerTemplate = $("#video-item").contents(
			".dialog__video-container"
		);
		setDialogContent.init.$videoItemTemplate = setDialogContent.init.$videoContainerTemplate.find(
			".dialog__video-item"
		);
		setDialogContent.init.$myBillTemplate = $("#my-bill").contents(
			".dialog__bill-wrap"
		);
		setDialogContent.init.$userBillTemplate = $("#customer-bill").contents(
			".dialog__bill-wrap"
		);
